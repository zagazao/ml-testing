...
auto benchmark(std::vector<std::vector<FEATURE_TYPE>> &X, std::vector<unsigned int> &Y, unsigned int n_classes, unsigned int repeat) {
	...
	#ifdef TREELITE
		std::vector<std::vector<Entry>> XTreelite;
    	//Entry * XEntry = new Entry[X.size()*n_features];
    	for (unsigned int i = 0; i < X.size(); ++i) {
			std::vector<Entry> entry(n_features);
    		for (unsigned int j = 0; j < n_features; ++j) {
				entry[j].fvalue = X[i][j];
    			//XEntry[j+n_features*i].fvalue = X[j+i*n_feaetures];
    		}
			XTreelite.push_back(entry);
    	}
    #endif
    ...
    for (unsigned int k = 0; k < repeat; ++k) {
	    ...
	    for (unsigned int i = 0; i < X.size(); ++i) {
			...
			#ifdef TREELITE
				Entry * x = &XTreelite[i][0];
				predict_multiclass(x, 0, output);
			#else
